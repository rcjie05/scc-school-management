...
/* ── NAVIGATION: fly-to & walk mode ── */
#btnWalk { }
#btnWalk.walk-active { background:rgba(232,255,60,.15);border-color:rgba(232,255,60,.5);color:var(--accent); }
...
#walkCrosshair svg { opacity:.7; }

/* walk HUD */
#walkHUD {
  position:absolute;bottom:40px;left:50%;transform:translateX(-50%);
  background:rgba(8,10,16,.92);backdrop-filter:blur(14px);
  border:1px solid rgba(232,255,60,.3);border-radius:50px;
  padding:6px 20px;display:none;align-items:center;gap:14px;
  font-family:'Space Mono',monospace;font-size:9px;color:var(--muted2);
  white-space:nowrap;z-index:200;letter-spacing:.6px;
}
#walkHUD.show { display:flex; }
#walkHUD kbd { background:var(--s2);border:1px solid var(--border2);border-radius:2px;padding:1px 5px;font-family:'Space Mono',monospace;font-size:8px;color:var(--text); }
#walkHUD .wh-label { color:var(--accent);font-weight:700;font-size:10px;letter-spacing:1.5px; }
...
@keyframes badgePop{from{opacity:0;transform:translateX(-50%) scale(.88)}to{opacity:1;transform:translateX(-50%) scale(1)}}

/* pointer-lock notice */
#lockNotice {
  position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  background:rgba(8,10,16,.95);border:1px solid rgba(232,255,60,.35);border-radius:10px;
  padding:24px 32px;text-align:center;z-index:400;display:none;
  font-family:'Barlow',sans-serif;
}
#lockNotice h3 { font-family:'Barlow Condensed',sans-serif;font-size:18px;font-weight:900;letter-spacing:2px;color:var(--accent);text-transform:uppercase;margin-bottom:8px; }
#lockNotice p { font-size:12px;color:var(--muted2);margin-bottom:16px;line-height:1.7; }
#lockNotice button { padding:8px 22px;border-radius:4px;border:none;background:var(--accent);color:#000;font-family:'Barlow',sans-serif;font-size:11px;font-weight:800;cursor:pointer;text-transform:uppercase;letter-spacing:.5px; }

/* mini map */
#miniMap {
  position:absolute;bottom:40px;right:14px;
  width:130px;height:100px;
  background:rgba(8,10,16,.85);border:1px solid var(--border2);border-radius:6px;
  overflow:hidden;display:none;z-index:150;
}
#miniMap canvas { width:100%;height:100%; }
#miniMap .mm-label { position:absolute;top:4px;left:6px;font-family:'Space Mono',monospace;font-size:7px;color:var(--muted);letter-spacing:1px;text-transform:uppercase; }
...
<div id="topbar">
  ...
  <div class="tb-tools">
    ...
    <button class="tb-btn" id="btnWalk" onclick="toggleWalkMode()" title="First-person walk mode">⬛ Walk</button>
  </div>
...
<div id="layout">
  <div id="wrap">
    ...
    <!-- Walk crosshair -->
    <div id="walkCrosshair">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <line x1="12" y1="4" x2="12" y2="9" stroke="#e8ff3c" stroke-width="1.5"/>
        <line x1="12" y1="15" x2="12" y2="20" stroke="#e8ff3c" stroke-width="1.5"/>
        <line x1="4" y1="12" x2="9" y2="12" stroke="#e8ff3c" stroke-width="1.5"/>
        <line x1="15" y1="12" x2="20" y2="12" stroke="#e8ff3c" stroke-width="1.5"/>
        <circle cx="12" cy="12" r="1.5" fill="#e8ff3c"/>
      </svg>
    </div>

    <!-- Walk HUD -->
    <div id="walkHUD">
      <span class="wh-label">WALK</span>
      <span><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> move</span>
      <span><kbd>mouse</kbd> look</span>
      <span><kbd>Esc</kbd> exit</span>
    </div>

    <!-- Pointer-lock click notice -->
    <div id="lockNotice">
      <h3>Walk Mode</h3>
      <p>Click anywhere on the canvas to<br>enable mouse-look navigation</p>
      <button onclick="requestPointerLock()">Click to Start</button>
    </div>
...
// ══════════════════════════════════════════════════════
// CAMERA: camUp (spherical → cartesian)
// ══════════════════════════════════════════════════════
function camUp(){
  camera.position.set(
...
// ══════════════════════════════════════════════════════
// FEATURE 1: FLY-TO / ROOM FOCUS
// ══════════════════════════════════════════════════════
let flyAnim = null; // { startSph, endSph, startTgt, endTgt, t, duration }

function flyToRoom(entry) {
  if (walkMode) return;
  const room = entry.room;
  const fi   = entry.fi;
  const isL  = room.shape === 'L';
  const cx   = isL ? room.x + room.w * 0.5 : room.x + room.w / 2;
  const cy   = fi * FH + (room.h || 60) / 2 + 6;
  const cz   = isL ? room.z + room.d * 0.5 : room.z + room.d / 2;

  const endTgt = new THREE.Vector3(cx, cy, cz);
  const dim    = Math.max(room.w, room.d);
  const endR   = Math.max(180, dim * 2.4);
  const endPhi = 0.55;

  flyAnim = {
    startTgt: tgt.clone(),
    endTgt,
    startSph: { theta: sph.theta, phi: sph.phi, r: sph.r },
    endSph:   { theta: sph.theta, phi: endPhi, r: endR },
    t: 0,
    duration: 0.65
  };
  ...
  badge.textContent = room.name + '  —  ' + room.w + '×' + room.d;
  badge.style.display = 'block';
  clearTimeout(badge._timer);
  badge._timer = setTimeout(() => badge.style.display = 'none', 2800);
}
...
function tickFly(dt) {
  if (!flyAnim) return;
  flyAnim.t = Math.min(flyAnim.t + dt / flyAnim.duration, 1);
  const e = easeInOut(flyAnim.t);
  tgt.lerpVectors(flyAnim.startTgt, flyAnim.endTgt, e);
  sph.phi   = flyAnim.startSph.phi   + (flyAnim.endSph.phi   - flyAnim.startSph.phi)   * e;
  sph.r     = flyAnim.startSph.r     + (flyAnim.endSph.r     - flyAnim.startSph.r)     * e;
  sph.theta = flyAnim.startSph.theta + (flyAnim.endSph.theta - flyAnim.startSph.theta) * e;
  camUp();
  if (flyAnim.t >= 1) flyAnim = null;
}

// Click on 3D mesh → fly to room
c3d.addEventListener('click', e => {
  if (walkMode) return;
  const moved = Math.abs(e.clientX - lm.x) + Math.abs(e.clientY - lm.y) > 5;
  if (moved) return;
  const rect = c3d.getBoundingClientRect();
  mouse.x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
  mouse.y = -((e.clientY - rect.top) / rect.height) * 2 + 1;
  raycaster.setFromCamera(mouse, camera);
  const hits = raycaster.intersectObjects(roomMeshes.map(e => e.mesh).filter(Boolean));
  if (hits.length) {
    const ud = hits[0].object.userData;
    const entry = roomMeshes.find(e => e.fi === ud.fi && e.ri === ud.ri);
    if (entry) { selectRoom(entry); flyToRoom(entry); }
  }
});

// Patch selectRoom to also fly on list-click
const _origSelectRoom = selectRoom;
function selectRoom(entry) {
  _origSelectRoom(entry);
  if (!walkMode) flyToRoom(entry);
}

// ══════════════════════════════════════════════════════
// FEATURE 2: WALK MODE (First-Person)
// ══════════════════════════════════════════════════════
let walkMode    = false;
let walkPos     = new THREE.Vector3();
let walkYaw     = 0;   // horizontal look (radians)
let walkPitch   = 0;   // vertical look (clamped)
let walkKeys    = {};
let pointerLocked = false;
const WALK_SPEED  = 120;  // units/sec
const WALK_EYE_H  = 52;   // eye height above floor
const WALK_PITCH_MAX = Math.PI / 2.8;

function toggleWalkMode() {
  if (walkMode) exitWalkMode(); else enterWalkMode();
}
...
function enterWalkMode() {
  ...
  // Start position: centre of active floor, or above ground
  const floorY = (FLOORS[activeFloor] ? activeFloor : 0) * FH + WALK_EYE_H;
  walkPos.set(tgt.x, floorY, tgt.z);
  walkYaw   = sph.theta + Math.PI;
  walkPitch = 0;

  // Make rooms translucent so you can see through walls
  roomMeshes.forEach(e => {
    if (e.mesh?.material) { e.mesh.material.opacity = 0.22; e.mesh.material.transparent = true; e.mesh.material.needsUpdate = true; }
    if (e.edgeMesh?.material) { e.edgeMesh.material.opacity = 0.15; e.edgeMesh.material.needsUpdate = true; }
  });

  document.getElementById('btnWalk').classList.add('walk-active');
  document.getElementById('walkCrosshair').style.display = 'block';
  document.getElementById('walkHUD').classList.add('show');
  document.getElementById('lockNotice').style.display = 'block';
  document.getElementById('miniMap').style.display = 'block';
  document.getElementById('c3d').style.cursor = 'none';
...
function exitWalkMode() {
  ...
  // Restore room opacity
  roomMeshes.forEach(e => {
    const isAct = e.fi === activeFloor;
    if (e.mesh?.material) { e.mesh.material.opacity = isAct ? 0.88 : 0.08; e.mesh.material.needsUpdate = true; }
    if (e.edgeMesh?.material) { e.edgeMesh.material.opacity = isAct ? 0.4 : 0.04; e.edgeMesh.material.needsUpdate = true; }
  });

  document.getElementById('btnWalk').classList.remove('walk-active');
  document.getElementById('walkCrosshair').style.display = 'none';
  document.getElementById('walkHUD').classList.remove('show');
  document.getElementById('lockNotice').style.display = 'none';
  document.getElementById('miniMap').style.display = 'none';
  document.getElementById('c3d').style.cursor = '';
  ...
  tgt.set(walkPos.x, FLOORS.length * FH / 2 || 0, walkPos.z);
  sph = { theta: walkYaw - Math.PI, phi: 0.72, r: 900 };
  camUp();
}
...
function onPointerLockChange() {
  ...
  document.getElementById('lockNotice').style.display = pointerLocked ? 'none' : (walkMode ? 'block' : 'none');
}

function onWalkKey(e) {
  walkKeys[e.code] = true;
  if (e.code === 'Escape') { exitWalkMode(); return; }
  // Click canvas to lock
  if (!pointerLocked && ['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code)) {
    c3d.requestPointerLock();
  }
  e.stopPropagation();
}
...
function tickWalk(dt) {
  ...
  // Forward / back
  if (walkKeys['KeyW'] || walkKeys['ArrowUp']) {
    walkPos.x += sinY * speed;
    walkPos.z += cosY * speed;
  }
  if (walkKeys['KeyS'] || walkKeys['ArrowDown']) {
    walkPos.x -= sinY * speed;
    walkPos.z -= cosY * speed;
  }
  // Strafe
  if (walkKeys['KeyA'] || walkKeys['ArrowLeft']) {
    walkPos.x -= cosY * speed;
    walkPos.z += sinY * speed;
  }
  if (walkKeys['KeyD'] || walkKeys['ArrowRight']) {
    walkPos.x += cosY * speed;
    walkPos.z -= sinY * speed;
  }

  // Clamp to building bounds with some margin
  const margin = 80;
  walkPos.x = Math.max(-margin, Math.min(BW + margin, walkPos.x));
  walkPos.z = Math.max(-margin, Math.min(BD + margin, walkPos.z));
...
function drawMiniMap() {
  const cnv = document.getElementById('mmCanvas');
  if (!cnv) return;
  const W = 130, H = 100;
  ...
  // Scale: building fits in canvas
  const scX = (W - 16) / BW;
  const scZ = (H - 16) / BD;
  const sc  = Math.min(scX, scZ);
  const ox  = 8, oz = 8;

  // Draw rooms on active floor
  const rooms = FLOORS[activeFloor]?.rooms || [];
  rooms.forEach(r => {
    ...
    ctx.lineWidth = 0.8;
    ctx.beginPath();
    ctx.rect(ox + r.x * sc, oz + r.z * sc, r.w * sc, r.d * sc);
    ctx.fill(); ctx.stroke();
  });
  ...
  ctx.lineWidth = 1;
  ctx.strokeRect(ox, oz, BW * sc, BD * sc);

  // Player dot
  const px = ox + walkPos.x * sc;
  const pz = oz + walkPos.z * sc;
  ...
  // Direction triangle
  const dl = 9;
  const dx = Math.sin(walkYaw) * dl;
  const dz = Math.cos(walkYaw) * dl;
...
(async function(){
  try{
    const res=await fetch('../php/api/registrar/get_floorplan3d.php',{credentials:'include'});
