...
<head>
    ...
    <link rel="apple-touch-icon" href="../images/logo2.jpg">
    <title>Attendance Tracking - Saint Cecilia College</title>
...
async function loadDailyAttendance() {
    ...
    const res  = await fetch(`../php/api/hr/get_attendance.php?date=${date}&role=${role}`);
...
async function saveAttendance() {
    ...
    const res  = await fetch('../php/api/hr/save_attendance.php', {
        method: 'POST',
...
async function loadMonthlySummary() {
    ...
    const res  = await fetch(`../php/api/hr/get_attendance.php?month=${month}&role=${role}&summary=1`);
    const data = await res.json();
...
function exportCSV() {
    ...
    if (currentView === 'daily' && date) {
        window.location.href = `../php/api/hr/export_attendance.php?date=${date}&role=${role}`;
    } else if (currentView === 'monthly' && month) {
        window.location.href = `../php/api/hr/export_attendance.php?month=${month}&role=${role}&summary=1`;
    }
}
